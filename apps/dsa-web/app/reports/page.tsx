...
import { listReports, type AnalysisRun } from "@/lib/api";
...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    listReports()
      ...
      .catch((e: unknown) => alive && (setRows([]), setError(e instanceof Error ? e.message : "Failed to load reports")));
    return () => {
      alive = false;
    };
  }, []);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Reports"
        description="Saved report runs with status, objective, and dataset preserved."
        actions={<Link href="/analysis" className={cn(buttonVariants({ size: "sm" }))}>New analysis →</Link>}
      />
      {error ? <ErrorState message={error} onRetry={() => window.location.reload()} /> : null}
      {rows === null ? (
        <Card className="p-5"><p className="text-sm text-zinc-500" aria-busy="true">Loading reports…</p></Card>
      ) : (
        <DataTable<AnalysisRun>
          ...
          emptyDescription="Run an analysis to generate your first downloadable report."
          ...
          columns={[
            ...
            { key: "dataset", header: "Dataset", render: (r) => <span className="font-mono text-xs text-zinc-600">{r.dataset ?? "—"}</span> },
            ...
            { key: "open", header: "Open", render: (r) => <Link href={`/analysis/${encodeURIComponent(r.runId ?? r.id)}`} className={cn(buttonVariants({ variant: "outline", size: "sm" }))}>Open →</Link> },
          ]}
        />
      )}
    </div>
